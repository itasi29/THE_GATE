...
private:
	...
	double CalcBSplineBasisFunc(const std::vector<double>& knot, unsigned int j, unsigned int p, double t)
	{
		...
		// 正値をとる範囲外ならゼロを返す
		if ((t < knot[j]) || (t > knot[j + static_cast<uint64_t>(p) + 1]))
			return (0);
		// p = 0 かつ knot[j] <= t <= knot[j + p + 1] なら 1 を返す
		if (p == 0)
			return (1);
		// p = 1 の場合、三角の頂点の値は特別扱い
		if (p == 1 && t == knot[static_cast<uint64_t>(j) + 1])
			return (1);
...
public:
		...
		mOrder(0)
	{
		...
		{
			dVector4 p0 = mControllPoint[static_cast<uint64_t>(i) - 1];
